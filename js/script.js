...
loggedCities = JSON.parse(localStorage.getItem("cityList")) || [];
cityBtn = document.querySelector(".searchedCities");
...
makeList = function () {
    //Creates a button from searched cities
    choice = document.querySelector("#searchedCity").value;
    searchedCity = document.createElement("button");
    searchedCity.className = "list-group-item";
    searchedCity.setAttribute("recentCity", choice);
...
//Create a function that will pass the previously seached cities to search term
cityBtn.onclick = function (recent) {
    recalledCity = recent.target.getAttribute("recentCity")
    fetch("https://api.openweathermap.org/data/2.5/weather?q="
            + recalledCity +
            "&appid=f19269e626e5152019a125e36257aaf9&units=imperial")
            .then(function (cityInfo) {
